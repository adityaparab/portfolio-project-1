...
| 2026-08-28 | Stack simplification: the optional compose-hosted LiteLLM proxy service is REMOVED — all model traffic goes to the external proxy configured in `.env` (`INVOICEOPS_LITELLM_BASE_URL`, e.g. a proxy hosted on another LAN machine). Fixed en route: the api service interpolated a different name (`API_LITELLM_BASE_URL`), so `.env` proxy settings were silently ignored in Compose; one variable name now works for containers and host-run scripts. `deploy/litellm/` alias configs remain for manually running a proxy (Phase 5 prod/eval). |
| 2026-08-28 | Agent Run redesigned as a Mantine vertical-Stepper master/detail view: stepper (left, path-filtered so Mantine's completed-rendering stays truthful; controlled — clicks select for inspection, progress derives from the run) + step output pane (right). Active step pulses via `[data-progress]` CSS and names its wire model in the description; LLM steps list the model in the stepper. 2 RTL tests cover path filtering, model-call inspection (reasoning + output), and pending status. |
| 2026-08-28 | **Model-call audit + step view on Agent Run**: every LLM round-trip (reasoning/thinking + final output + alias/wire-model + prompt version + latency) is persisted to a new append-only `model_calls` table (migration 0005) via a gateway observer correlated to run/stage by contextvar — recording is best-effort and can never fail a call; served by `GET /v1/runs/{id}/model-calls`. Trace gained `active_node` (blinks only while RUNNING — paused/settled runs have none) and per-stage alias/model/prompt-version metadata. Agent Run screen is now a clickable step view: completed steps green, active step with pulsing live dot + model badge, pending dashed; clicking shows the model output (+ reasoning when the backend emits it) or the pending/running status, plus the step's ledger payload. Prompts bumped extract@v3 + triage@v2: respond ONLY in English. En-route fixes found live: migration 0006 grants sequence USAGE for post-0002 tables (app-role INSERTs failed; invisible in CI since testcontainers runs as owner); extraction coerces model-emitted `""` optional fields to None (live glm-ocr output failed date validation). |
| 2026-08-28 | **Bug fix — background pipeline never ran**: the post-upload task read the graph runner off the FastAPI app object instead of `app.state`, so every upload since #84 stayed QUEUED at `ingest.accepted` while looking healthy (caught live on the single-port deploy; integration tests drive the runner directly and the e2e only asserted the receipt). Fixed the accessor; `readyz` gained a `pipeline` check so a dead runner is visible; the e2e ingest test now asserts `run.started` lands in the ledger (environment-independent — whatever the model backend does afterward). En route findings: exact-duplicate uploads in repeated e2e runs now carry unique bytes; the user's `glm-ocr` backend returns HTTP 500 for PDFs sent as `image_url` (PNG/JPEG vision confirmed working) — PDF invoices retry then DLQ on that backend. |
| 2026-08-28 | **Single-command deploy**: the api image builds the UI (node stage, `VITE_API_BASE_URL=""` → same-origin `/v1/*` calls) and serves the built SPA on :8000 — root + client-route fallback (React Router deep links), immutable caching for hashed assets, no-cache index.html, `/api/*` 404s, path-traversal guard; API routes keep precedence over the catch-all. `docker compose up -d --build api` is now the whole app (seeding stays separate). Frontend API base is build-time configurable (`apiBaseUrl`), the upload hook uses it; CI e2e tests the :8000 single-port path; the `ui` service remains dev-only hot reload. Verified live: SPA + assets + API on :8000 and Playwright green against the deployment. |
