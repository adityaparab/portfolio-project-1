"""Extraction prompt — version extract@v3.

Changes vs v2: respond ONLY in English regardless of the document language
(audit trail + eval normalization).
Changes vs v1 (issue #25): the contract grows ``po_number`` — the PO
reference printed on the document — which the 3-way match (#21) keys on.
...
Rules:
- Respond ONLY in English, whatever language the document uses.
...
- `po_number` is the purchase-order reference printed on the document
  (e.g. "PO-2026-00042"), null if none is printed. Copy it exactly.
